...
int getWordId(const string &word)
{
    if (wordToId.find(word) == wordToId.end())
    {
        ...
        wordGraph.emplace_back(); // 新增空邻接链表
    }
    return wordToId[word];
}

// 单句内两两单词统计共现（无向，避免a-b、b-a重复）
...
void menu() {
    ...
    cout << "8. 查找替换" << endl;
    cout << "9. 语法检查(V3)" << endl;
    cout << "10. 全文子串查找(V4-子串匹配)" << endl;
    cout << "11. 关键词高亮显示(V4-高亮)" << endl;
    cout << "12. 生成词共现关系图(V6-图结构)" << endl;
    cout << "0. 退出" << endl;
    cout << "请输入选项：";
}
