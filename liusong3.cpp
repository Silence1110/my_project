...
#include <cstring>
#include <cctype>
#include <cstdio>   //补上sprintf/memset头文件
using namespace std;
...
struct StrArr{
    ...
    void expand(){
        cap*=2;
        char** t=new char*[cap];
        for(int i=0;i<size;i++) t[i]=data[i];
        delete[]data;data=t;
    }
    void push(const char*s){
        if(size>=cap) expand();
        int l=strlen(s);
        data[size]=new char[l+1];
        strcpy(data[size++],s);
    }
...
struct KV{
    ...
    int val;
    bool empty;
    KV(){empty=1;val=0;memset(key,0,64);}
};
...
struct HashTable{
    ...
    int getHash(const char*s){
        int r=0;
        for(;*s;s++) r=(r*131+*s)%HASH_MOD;
        return r;
    }
    void insert(const char*s){
        int p=getHash(s);
        while(!table[p].empty && strcmp(table[p].key,s))
            p=(p+1)%HASH_MOD;
        if(table[p].empty){
            strcpy(table[p].key,s);
            table[p].empty=0;
            table[p].val=1;
        }else table[p].val++;
    }
    ...
    void getAll(StrArr&k,StrArr&v){
        char buf[20];
        for(int i=0;i<HASH_MOD;i++){
            if(!table[i].empty){
                k.push(table[i].key);
                sprintf(buf,"%d",table[i].val);
                v.push(buf);
            }
        }
    }
...
HashTable countFreq(StrArr&w){
    HashTable ht;
    for(int i=0;i<w.size;i++) ht.insert(w[i]);
    return ht;
}
...
void grammarCheck(const char* txt,StrArr&words){
    ...
    for(int i=0;i<words.size;i++){
        if(!dict.exist(words[i])) cout<<words[i]<<" ";
    }
    cout<<"\n【句首大写检查】\n";
...
//新增主函数，程序入口
int main(){
    char test[]="this Is a apple. he like cat?";
    char* raw=preProcess(test);
    StrArr wd=splitWords(raw);
    grammarCheck(test,wd);
    delete[]raw;
    return 0;
}
